...
async def manual_trading_handler(message: Message, state: FSMContext):
    ...
    if user:
        if user.state == 3:
            return
        if not user.is_paid:
            user.top_up_date = datetime.datetime.now(
                google_sheet.moscow_timezone
            ).strftime("%d/%m/%Y, %H:%M:%S")
            await user.save()
            google_sheet.update_top_up(user.user_id, user.top_up_date)
            await message.answer(
                language.for_pay[config.LANG].format(user_id=message.from_user.id)
            )
            return
        await message.answer(
...
async def trade_tools_callback(callback_query: CallbackQuery, state: FSMContext):
    ...
    if user:
        ...
        await callback_query.message.answer(
            language.options[config.LANG],
            reply_markup=get_inline_keyboard(trade_time, pre="tradetime"),
        )
...
async def trade_time_callback(callback_query: CallbackQuery, state: FSMContext):
    ...
    if user and user.state == 2:
        ...
        user.trade_choose_tools = trade_choose_tools
        await callback_query.message.answer(
            language.bot_analizing_please_wait[config.LANG]
        )
        if config.DECISION is None:
    ...
    if user:
        if user.state >= 2:
            ...
            inline_keyboard = get_inline_keyboard(
                language.auto_trading_signals_time[config.LANG],
                custom=["auto_time_2", "auto_time_4", "auto_time_5"],
            )
            await message.answer(
...
async def win_handler(callback_query: CallbackQuery):
    ...
    if user and user.state == 4:
        ...
        google_sheet.update_lose_win_count(
            user.user_id, user.win_count - user.last_lose_count
        )
...
async def handle_trader_agree_auto(callback_query: CallbackQuery):
    ...
    user = await User.get_or_none(user_id=callback_query.from_user.id)
    if user:
        is_member = await check_subscription(
            callback_query.bot, callback_query.from_user.id
        )
        if not is_member:
            await callback_query.message.answer(
                language.new_sub_to_channel[config.LANG],
                reply_markup=get_inline_keyboard(
                    language.i_have_subscribed[config.LANG],
                    custom=["i_have_subscribed"],
                ),
            )
            return
        if user.state == 0:
            await callback_query.message.bot.send_message(
                callback_query.from_user.id, language.send_trader_id[config.LANG]
            )
            user.state = 1
            await user.save()
        else:
