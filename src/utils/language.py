tiime_options = {
    "RU": [
        "5 секунд",
        "15 секунд",
        "30 секунд",
        "1 минута",
        "5 минуты",
        "10 минуты",
        "15 минуты",
    ],
    "EN": [
        "5 seconds",
        "15 seconds",
        "30 seconds",
        "1 minute",
        "5 minutes",
        "10 minutes",
        "15 minutes",
    ],
    "ES": [
        "5 segundos",
        "15 segundos",
        "30 segundos",
        "1 minuto",
        "5 minutos",
        "10 minutos",
        "15 minutos",
    ],
    "AR": [
        "5 ثواني",
        "15 ثانية",
        "30 ثانية",
        "1 واحدة",
        "5 دقائق",
        "10 دقائق",
        "15 دقيقة",
    ],
    "TR": [
        "5 saniye",
        "15 saniye",
        "30 saniye",
        "1 dakika",
        "5 dakika",
        "10 dakika",
        "15 dakika",
    ],
    "ID": [
        "5 detik",
        "15 detik",
        "30 detik",
        "1 menit",
        "5 menit",
        "10 menit",
        "15 menit",
    ],
}

ftm = {
    "RU": "Знакомьтесь с нашим уникальным торговым ботом, который стал настоящим спасением для многих инвесторов! Этот инновационный инструмент работает на уровне высочайшей точности, предоставляя своим пользователям эксклюзивные сигналы по трейдингу бинарными опционами. Независимо от вашего уровня опыта - начинающий трейдер или профессионал, наш бот поможет вам достичь финансового успеха.\nС помощью передовых технологий и аналитических данных, наш бот позволяет делать высокодоходные сделки, а его интеллектуальная система анализа рынка предсказывает будущие тренды с невероятной точностью. Надежность, эффективность и оперативность - вот ключевые преимущества нашего бота.",
    "EN": "Meet our unique trading bot, which has become a true lifesaver for many investors! This innovative tool operates with the highest level of precision, providing exclusive signals for binary options trading. Whether you’re a beginner or a seasoned trader, our bot can help you achieve financial success.\nUsing advanced technology and analytical data, our bot enables high-yield trades, and its intelligent market analysis system predicts future trends with incredible accuracy. Reliability, efficiency, and promptness are the key advantages of our bot.",
    "ES": "Conoce a nuestro exclusivo bot de trading, que se ha convertido en un verdadero salvavidas para muchos inversores. Esta innovadora herramienta funciona con una precisión excepcional, proporcionando señales exclusivas para operar con opciones binarias. Sin importar tu nivel de experiencia, ya seas un trader principiante o un profesional, nuestro bot te ayudará a alcanzar el éxito financiero.\nGracias a tecnologías avanzadas y datos analíticos, nuestro bot permite realizar operaciones altamente rentables, y su sistema inteligente de análisis de mercado predice las tendencias futuras con una precisión increíble. Confiabilidad, eficiencia y rapidez son las principales ventajas de nuestro bot.",
    "AR": "تعرف على بوت التداول الفريد الخاص بنا، والذي أصبح خلاصًا حقيقيًا للعديد من المستثمرين! تعمل هذه الأداة المبتكرة على أعلى مستوى من الدقة، وتوفر لمستخدميها إشارات تداول الخيارات الثنائية الحصرية. بغض النظر عن مستوى خبرتك - سواء كنت متداولًا مبتدئًا أو محترفًا، سيساعدك البوت الخاص بنا على تحقيق النجاح المالي.\nباستخدام أحدث التقنيات والبيانات التحليلية، يتيح لك البوت الخاص بنا إجراء عمليات تداول مربحة للغاية، ويتنبأ نظام تحليل السوق الذكي الخاص به بالاتجاهات المستقبلية بدقة مذهلة. الموثوقية والكفاءة هي المزايا الرئيسية لروبوتنا.",
    "TR": "Tanışın! Birçok yatırımcı için gerçek bir kurtarıcı olan benzersiz ticaret botumuz burada! Bu yenilikçi araç, kullanıcılarına ikili opsiyon ticareti için özel sinyaller sağlayarak yüksek doğruluk seviyesinde çalışır. İster yeni başlayan ister profesyonel bir yatırımcı olun, botumuz sizi finansal başarıya ulaştırmanıza yardımcı olacaktır.\nGelişmiş teknoloji ve analitik veri desteği ile botumuz, yüksek kârlı işlemler yapmanıza olanak tanırken, piyasa analizi sistemi gelecekteki trendleri inanılmaz bir doğrulukla tahmin eder. Güvenilirlik, verimlilik ve hız, botumuzun başlıca avantajlarıdır.",
    "ID": "Temui bot perdagangan unik kami, yang telah menjadi penyelamat nyata bagi banyak investor! Alat inovatif ini bekerja pada tingkat akurasi tertinggi, memberikan penggunanya sinyal perdagangan opsi biner eksklusif. Terlepas dari tingkat pengalaman Anda - trader pemula atau profesional, bot kami akan membantu Anda mencapai kesuksesan finansial.\nDengan bantuan teknologi canggih dan data analitis, bot kami memungkinkan Anda melakukan perdagangan yang sangat menguntungkan, dan sistem analisis pasar cerdasnya memprediksi tren masa depan dengan akurasi luar biasa. Keandalan, efisiensi, dan efisiensi adalah keunggulan utama bot kami.",
}

you_paid = {
    "RU": "Теперь вам активна функция трейдинга в ручном режиме что бы начать торговать нажмите кнопку опции",
    "EN": "Now you have access to manual trading mode. To start trading, simply click the ‘Options’ button.",
    "ES": "Ahora tienes habilitada la función de trading manual. Para comenzar a operar, simplemente presiona el botón de opciones.",
    "AR": "الآن أصبح خيار التداول اليدوي نشط لبدء التداول، انقر فوق زر الخيارات",
    "TR": 'Artık manuel ticaret modu sizin için etkin. Ticaret yapmaya başlamak için "Opsiyonlar" düğmesine basın.',
    "ID": 'Fitur perdagangan manual sekarang aktif untuk Anda. Untuk mulai berdagang, tekan tombol "Opsi".',
}

indicator_form = {
    "RU": "💱Валютная пара:({trade_tools})\n⏳Время эксперации:({trade_time_str})\n\n\n✅ Бот рекомендует открывать торги на {trade_direction} \nВремя на использование сигнала: 15 секунд",
    "EN": "💱Currency Pair: ({trade_tools})\n⏳Expiration Time: ({trade_time_str})\n\n\n✅ The bot recommends opening trades for {trade_direction}\nTime to use the signal: 15 seconds",
    "ES": "💱Par de divisas: ({trade_tools})\n⏳Hora de expiración: ({trade_time_str})\n\n\n✅ El bot recomienda abrir operaciones al {trade_direction}\nTiempo para usar la señal: 15 segundos",
    "AR": "💱زوج العملات: ({trade_tools})\n⏳زمن إنتهاء الصلاحية: ({trade_time_str})\n\n\n✅ يوصي البوت بفتح صفقة علي {trade_direction}\nوقت استخدام الإشارة: 15 ثانية",
    "TR": "💱 Döviz Çifti: ({trade_tools})\n⏳Vade Süresi: ({trade_time_str})\n\n\n✅ Bot, {trade_direction} yapmanızı öneriyor.\nSinyali kullanmak için süreniz 15 saniye",
    "ID": "💱Pasangan mata uang: ({trade_tools})\n⏳Waktu kedaluwarsa: ({trade_time_str})\n\n\n✅Bot merekomendasikan pembukaan perdagangan untuk {trade_direction}\nWaktu untuk menggunakan sinyal 15 detik",
}

lose_text = {
    "RU": "Бот рекомендует вам сделать паузу 5 минут, вы проигрывайте из за разницы в тайм фрейме вашего устройства и бота!\nЧерез 5 минут вам нужно выбрать новую трейдинговую пару обновить страницу браузера и подтвердить обновление и выбор новой пары для трейдинга !",
    "EN": "The bot recommends that you take a 5-minute break; you are losing due to the time frame difference between your device and the bot!\nAfter 5 minutes, you need to choose a new trading pair, refresh the page, and confirm the update and selection of the new pair for trading!",
    "ES": "El bot te recomienda tomar una pausa de 5 minutos, ya que podrías estar perdiendo debido a la diferencia en el marco de tiempo entre tu dispositivo y el bot. Después de 5 minutos, debes seleccionar un nuevo par de trading, actualizar la página del navegador y confirmar la elección del nuevo par para operar.",
    "AR": "يوصي البوت بالتوقف لمدة 5 دقائق، لأنك قد تخسر بسبب اختلاف الإطار الزمني لجهازك والبوت!\nبعد 5 دقائق، تحتاج إلى تحديد زوج تداول جديد، وتحديث صفحة المتصفح وتأكيد التحديث واختيار زوج جديد للتداول.",
    "TR": "Bot, cihazınız ile bot arasındaki zaman dilimi farkından dolayı kaybettiğinizi belirterek 5 dakika ara vermenizi öneriyor! 5 dakika sonra yeni bir ticaret çifti seçmeli, tarayıcı sayfasını yenilemeli ve güncellemeyi ve yeni ticaret çiftini onaylamalısınız!",
    "ID": "Bot menyarankan Anda untuk istirahat selama 5 menit, karena Anda kalah akibat perbedaan waktu antara perangkat Anda dan bot! Setelah 5 menit, Anda perlu memilih pasangan perdagangan baru, memperbarui halaman browser, dan mengonfirmasi pembaruan serta pemilihan pasangan perdagangan baru!",
}

sub_to_channel = {
    "RU": "Подпишитесь на канал (напишите @bot_helpmanager в личку: {user_id} , я выдам доступ)",
    "EN": "Subscribe to the channel (send @bot_helpmanager a private message: {user_id}, and I will grant access)",
    "ES": "Suscríbete al canal (@bot_helpmanager un mensaje privado: {user_id}, y te daré acceso)",
    "AR": "اشترك في القناة (أرسل لي رسالة خاصة: {user_id} وسأمنحك الوصول)",
    "TR": "Kanalımıza abone olun (özel mesaj olarak @bot_helpmanager 'a mesaj atın: {user_id}, erişim izni vereceğim)",
    "ID": "Berlangganan ke saluran (kirimi saya pesan pribadi: {user_id}, dan saya akan memberikan akses)",
}

new_sub_to_channel = {
    "RU": "Подпишитесь на канал, чтобы продолжить",
    "EN": "Subscribe to the channel to continue",
    "ES": "Suscríbete al canal para continuar",
    "AR": "اشترك في القناة للمتابعة",
    "TR": "Devam etmek için kanala abone olun",
    "ID": "Berlangganan ke saluran untuk melanjutkan",
}

i_have_subscribed = {
    "RU": "Я подписался",
    "EN": "I have subscribed",
    "ES": "Me he suscrito",
    "AR": "لقد اشتركت",
    "TR": "Abone oldum",
    "ID": "Saya telah berlangganan",
}

send_trader_id = {
    "RU": "Отправьте ваш ид пользователя в таком формате: 123456789",
    "EN": "Send your user ID in this format: 123456789",
    "ES": "Envía tu ID de usuario en este formato: 123456789",
    "AR": "أرسل رمز المستخدم الخاص بك بهذا التنسيق: 123456789",
    "TR": "Kullanıcı ID'nizi şu formatta gönderin: 123456789",
    "ID": "Kirim ID pengguna Anda dalam format berikut: 123456789",
}


menu = {
    "RU": "Меню:",
    "EN": "Menu:",
    "ES": "Menú:",
    "AR": "القائمة:",
    "TR": "Menü:",
    "ID": "Menu:",
}

default_seconds = {
    "RU": "15 секунд",
    "EN": "15 seconds",
    "ES": "15 segundos",
    "AR": "15 ثانية",
    "TR": "15 saniye",
    "ID": "15 detik",
}

trade_direction = {
    "RU": ["Понижение📉", "Повышение📈"],
    "EN": ["Decrease📉", "Increase📈"],
    "ES": ["Disminución📉", "Aumento📈"],
    "AR": ["الهبوط📉", "الصعود📈"],
    "TR": ["Alım 📈",  "Satım 📉"],
    "ID": ["Kenaikan 📈", "Penurunan 📉"]

}

trade_result_question = {
    "RU": "Какой вы получили результат по последней сделке ({trade_tool})?",
    "EN": "What result did you get from your last trade ({trade_tool})?",
    "ES": "¿Cuál fue el resultado de tu última operación ({trade_tool})?",
    "AR": "ما هي النتيجة التي حصلت عليها في آخر تداول لك ({trade_tool})؟",
    "TR": "Son işlemdeki sonucunuz ne oldu ({trade_tool})?",
    "ID": "Apa hasil perdagangan terakhir ({trade_tool})?",
}


choose_trade_pair = {
    "RU": "Выберите торговую пару : {random_trade_tool}\nВ опции: {random_trade_type}\nВремя эксперации: {random_trade_time_str}",
    "EN": "Select a trading pair: {random_trade_tool}\nIn options: {random_trade_type}\nExpiration Time: {random_trade_time_str}",
    "ES": "Selecciona un par de divisas: {random_trade_tool}\nEn las opciones: {random_trade_type}\nHora de expiración: {random_trade_time_str}",
    "AR": "حدد زوج تداول: {random_trade_tool}\nفي الخيارات: {random_trade_type}\nزمن انتهاء الصلاحية: {random_trade_time_str}",
    "TR": "İşlem çiftini seçin: {random_trade_tool}\nSeçeneklerde: {random_trade_type}\nVade Süresi: {random_trade_time_str}",
    "ID": "Pilih pasangan perdagangan: {random_trade_tool}\nDalam opsi: {random_trade_type}\nWaktu kedaluwarsa: {random_trade_time_str}",
}

confirm_choice = {
    "RU": "Подтверждаю выбор нужных данных!",
    "EN": "I confirm the selection of the required data!",
    "ES": "Confirmo la selección de los datos requeridos!",
    "AR": "أؤكد اختيار البيانات المطلوبة!",
    "TR": "Gerekli verileri seçtiğimi onaylıyorum!",
    "ID": "Mengonfirmasi pemilihan data yang tepat!",
}

bot_analizing_please_wait = {
    "RU": "Бот Анализирует рынок, Пожалуйста подождите",
    "EN": "The bot is analyzing the market, please wait.",
    "ES": "El bot está analizando el mercado, por favor espere.",
    "AR": "البوت يقوم بتحليل السوق، يرجى الانتظار.",
    "TR": "Bot piyasayı analiz ediyor, lütfen bekleyin.",
    "ID": "Bot sedang menganalisis pasar, harap tunggu.",
}

win_wait = {
    "RU": "Ожидайте в течении 2-10 минут бот отправит вам новый подходящий сигнал",
    "EN": "Please wait 2-10 minutes. The bot will send you a new suitable signal.",
    "ES": "Por favor, espere de 2 a 10 minutos. El bot te enviará una nueva señal adecuada.",
    "AR": "انتظر من 2 إلى 10 دقائق. سيقوم البوت بإرسال إشارة مناسبة جديدة.",
    "TR": "Lütfen 2-10 dakika bekleyin. Bot size yeni bir uygun sinyal gönderecek.",
    "ID": "Harap tunggu 2-10 menit. Bot akan mengirimkan sinyal baru yang sesuai.",
}

lose_wait = {
    "RU": "Ожидайте бот сейчас анализирует в чем возникла проблема и рекомендует вам перезапустить страницу браузера для лучшей синхронизации",
    "EN": "Please wait. The bot is analyzing the issue and recommends that you refresh the browser page for better synchronization.",
    "ES": "Por favor, espere. El bot está analizando el problema y recomienda que actualices la página del navegador para una mejor sincronización.",
    "AR": "انتظر. يقوم البوت الآن بتحليل المشكلة ويوصي بإعادة تحميل صفحة المتصفح للحصول على مزامنة أفضل.",
    "TR": "Bot şu anda sorunun ne olduğunu analiz ediyor ve daha iyi senkronizasyon için tarayıcı sayfasını yeniden başlatmanızı öneriyor..",
    "ID": "Mohon tunggu, bot sekarang menganalisis apa masalahnya dan merekomendasikan Anda untuk meluncurkan ulang halaman browser untuk sinkronisasi yang lebih baik",
}

i_have_updated_site = {
    "RU": "Я обновил страницу Браузера",
    "EN": "I have refreshed the browser page.",
    "ES": "He actualizado la página del navegador.",
    "AR": "لقد قمت بتحديث صفحة المتصفح.",
    "TR": "Tarayıcı sayfasını yeniledim.",
    "ID": "SSaya me-refresh halaman browser.",
}

manual_trading = {
    "RU": "Ручной трейдинг",
    "EN": "Manual Trading",
    "ES": "Trading Manual",
    "AR": "التداول اليدوي",
    "TR": "Manuel Ticaret",
    "ID": "Perdagangan Manual",
}

trading_methods = {
    "RU": ["Ручной трейдинг", "Управляемый трейдинг"],
    "EN": ["Manual Trading", "Managed Trading"],
    "ES": ["Trading Manual", "Trading Gestionado"],
    "AR": ["التداول اليدوي", "التداول المدعوم"],
    "TR": ["Manuel Ticaret", "Yönetilen Ticaret"],
    "ID": ["Perdagangan Manual", "Trading yang dikelola"],
}

auto_trading = {
    "RU": "Управляемый трейдинг",
    "EN": "Managed Trading",
    "ES": "Trading Gestionado",
    "AR": "التداول المدعوم",
    "TR": "Yönetilen Ticaret",
    "ID": "Trading yang dikelola",
}

auto_trading_signals_time = {
    "RU": [
        "10 минут (2 сигнала)",
        "20 минут (4 сигнала)",
        "30 минут Рекомендация! (5 сигналов)",
    ],
    "EN": [
        "10 minutes (2 signals)",
        "20 minutes (4 signals)",
        "30 minutes (Recommended! 5 signals)",
    ],
    "ES": [
        "10 minutos (2 señales)",
        "20 minutos (4 señales)",
        "30 minutos (¡Recomendado! 5 señales)",
    ],
    "AR": ["١٠ دقائق (٢ إشارات)", "٢٠ دقيقة (٤ إشارات)", "٣٠ دقيقة (توصية! ٥ إشارات)"],
    "TR": [
        "10 dakika (2 sinyal)",
        "20 dakika (4 sinyal)",
        "30 dakika (Önerilen! 5 sinyal)",
    ],
    "ID": [
        "10 menit (2 sinyal)",
        "20 menit (4 sinyal)",
        "30 menit (Rekomendasi! 5 sinyal)",
    ],
}

auto_trading_question_count = {
    "RU": "Выберите сколько у вас есть времени для торговой сессии? (с суммой депозита до 50 бакс вы можете использовать бота 2 раз в сутки)",
    "EN": "Choose how much time you have for your trading session? (With a deposit amount up to $50, you can use the bot twice a day)",
    "ES": "Elige cuánto tiempo tienes para tu sesión de trading. (Con un depósito de hasta $50, puedes usar el bot dos veces al día)",
    "AR": "اختر كم من الوقت لديك لجلسة التداول؟ (مع إيداع يصل إلى 50 دولارًا، يمكنك استخدام البوت مرتين في اليوم)",
    "TR": "Ticaret seansı için ne kadar zamanınız var? (50 $ 'a kadar depozito tutarı ile botu günde iki kez kullanabilirsiniz)",
    "ID": "Pilih berapa banyak waktu yang Anda miliki untuk sesi trading? Dengan jumlah setoran hingga $50, Anda dapat menggunakan bot dua kali sehari",
}

options = {
    "RU": "Опции:",
    "EN": "Options:",
    "ES": "Opciones:",
    "AR": "الخيارات:",
    "TR": "Seçenekler:",
    "ID": "Opsi:",
}

analise_found_5 = {
    "RU": "Анализ рынка выявил 5 наилучших торговых пар:",
    "EN": "Market analysis found 5 best trading pairs:",
    "ES": "El análisis del mercado encontró 5 mejores pares de divisas:",
    "AR": "تحليل السوق وجد 5 أفضل أزواج تداول:",
    "TR": "Piyasa analizi en iyi 5 işlem çiftini buldu:",
    "ID": "Analisis pasar menemukan 5 pasangan trading terbaik:",
}

win = {
    "RU": "Выигрыш",
    "EN": "Win",
    "ES": "Ganancia",
    "AR": "ربح",
    "TR": "Kazanç",
    "ID": "Kemenangan",
}

trade_result_types = {
    "RU": ["Выигрыш", "Проигрыш"],
    "EN": ["Win", "Lose"],
    "ES": ["Ganancia", "Pérdida"],
    "AR": ["ربح", "خسارة"],
    "TR": ["Kazanç", "Kayıp"],
    "ID": ["Kemenangan", "Rugi"],
}

win_amount = {
    "RU": "Напишите сумму выигрыша",
    "EN": "Please write the amount of your profit.",
    "ES": "Indica la cantidad de ganancia.",
    "AR": "اكتب مبلغ الربح",
    "TR": "Kazanç miktarını girin",
    "ID": "Tulislah jumlah keuntungan Anda",
}

lose = {
    "RU": "Проигрыш",
    "EN": "Lose",
    "ES": "Pérdida",
    "AR": "خسارة",
    "TR": "Kayıp",
    "ID": "Rugi",
}

lose_confirm_refresh = {
    "RU": "Я подтверждаю обновление страницы и выбор новой пары для трейдинга !",
    "EN": "I confirm the page update and the choice of a new pair for trading!",
    "ES": "Confirmo la actualización de la página y la selección de un nuevo par para operar!",
    "AR": "أؤكد تحديث الصفحة واختيار زوج جديد للتداول!",
    "TR": "Sayfa yenilemesini ve yeni bir ticaret çifti seçimini onaylıyorum!",
    "ID": "Saya mengonfirmasi pembaruan halaman dan pemilihan pasangan baru untuk trading!",
}

back = {
    "RU": "Назад в Меню",
    "EN": "Back to Menu",
    "ES": "Volver al menú",
    "AR": "العودة إلى القائمة",
    "TR": "Menüye geri dön",
    "ID": "Kembali ke Menu",
}

wait_trading_done = {
    "RU": "Пока ваша торговля не окончена вы не можете использовать новые сигналы!",
    "EN": "While your trade is not yet completed, you cannot use new signals!",
    "ES": "Mientras tu operación no esté cerrada, no puedes usar nuevas señales!",
    "AR": "حتى اكتمال تداولك، لا يمكنك استخدام إشارات جديدة!",
    "TR": "Ticaretiniz tamamlanmadan yeni sinyalleri kullanamazsınız!",
    "ID": "Sampai perdagangan Anda selesai, Anda tidak dapat menggunakan sinyal baru!",
}

click_win_or_lose = {
    "RU": "Нажмите на кнопку Выигрыш или Проигрыш",
    "EN": "Press the Win or Loss button",
    "ES": "Presiona el botón de Ganancia o Pérdida",
    "AR": "اضغط على زر الربح أو الخسارة",
    "TR": "Kazanç veya Kayıp butonuna tıklayın",
    "ID": "Tekan tombol Menang atau Kalah",
}


for_pay = {
    "RU": "Что бы получить доступ к ручному трейдингу, вам нужно оплатить подписку стоимостью 50 долларов. После того как вы оплатите, отправьте ид пользователя ({user_id}) и скриншот менеджеру, тогда вам будет доступна функция ручного трейдинга.\n(@bot_helpmanager)",
    "EN": "Manual trading has new profitable features such as the selection of a variety of trading tools.\nYou can also trade now:\n✅ Cryptocurrencies\n✅ Commodities\n✅ Stock\n✅ Indexes\nAnd much more.\n\n- You can choose a specific currency pair and the timeframe you need - regardless of the time of day and day of the week, allowing you to trade and increase your balance at any convenient time.\n\n- It is much faster and more efficient because, based on the news background of events occurring globally, you can trade on assets with the highest volatility.\n\n- Using manual trading, you will gain access to a powerful analytical tool that enables informed decisions and minimizes risks!\n\nTo get access to manual trading, you need to pay for a subscription worth $50. After making the payment, send your user ID ({user_id}) and a screenshot to the manager. Then you will have access to the manual trading function.\n(@bot_helpmanager)",
    "ES": "El trading manual tiene características rentables, como la selección de una variedad de herramientas de trading.  \nTambién puedes operar ahora:  \n✅ Criptomonedas  \n✅ Commodities  \n✅ Acciones  \n✅ Índices  \nY mucho más.\n\n- Puedes elegir un par de divisas específico y el marco de tiempo que necesites, sin importar la hora del día ni el día de la semana, lo que te permite operar e incrementar tu saldo en cualquier momento conveniente.\n\n- Es mucho más rápido y eficiente, porque, basándote en las noticias y los eventos globales, puedes operar con activos de alta volatilidad.\n\n- Al utilizar el trading manual, tendrás acceso a una poderosa herramienta analítica que te permitirá tomar decisiones informadas y minimizar riesgos.\n\nPara acceder al trading manual, debes hacer un segundo depósito de $30 en tu cuenta de trading (el depósito debe ser exactamente el segundo, si ya tienes un saldo de $30, no cuenta).  \nDespués de hacer el depósito, envía este ID de usuario especial ({user_id}) y una captura de pantalla al  \ngerente. Luego tendrás acceso a la función de trading manual.  \n(@bot_helpmanager)",
    "AR": "يتضمن التداول اليدوي ميزات جديدة مربحة، مثل اختيار مجموعة متنوعة من أدوات التداول.\n\nيمكنك الآن التداول على:\n✅ العملات المشفرة\n✅ السلع\n✅ الأسهم\n✅ المؤشرات\nو أكثر من ذلك بكثير.\n\nكما يمكنك اختيار زوج عملات محدد والإطار الزمني الذي تحتاجه - بغض النظر عن وقت اليوم أو يوم الأسبوع، مما يسمح لك بالتداول وزيادة رصيدك في أي وقت مناسب.\n\nإنه أسرع وأكثر كفاءة بكثير، لأنه بناءً على الخلفية الإخبارية للأحداث التي تحدث في العالم - يمكنك التداول على الأصول التي تتمتع بأعلى تقلب.\n\nباستخدام التداول اليدوي - ستحصل على وصول إلى أداة تحليلية قوية ستسمح لك باتخاذ قرارات مدروسة وتقليل المخاطر!\n\nللحصول على حق الوصول إلى التداول اليدوي، تحتاج إلى دفع اشتراك بقيمة 50 دولارًا. بعد إجراء الدفع، أرسل معرف المستخدم الخاص بك ({user_id}) ولقطة شاشة إلى المسؤول. بعد ذلك، سيكون لديك حق الوصول إلى وظيفة التداول اليدوي.\n(@bot_helpmanager)",
    "TR": "Manuel ticarete erişim sağlamak için 50 dolar depozito yatırmanız gerekmektedir! Depozito yaptıktan sonra, kullanıcı kimliğinizi ve ekran görüntüsünü yöneticinize gönderin ({user_id}), ardından manuel ticaret özelliği sizin için aktif hale gelecektir.\n(@bot_helpmanager)",
    "ID": "Trading Manual Punya Fitur Menguntungkan! 🚀\n\nDengan trading manual, kamu bisa memilih banyan alat trading yang tersedia untuk memaksimalkan profit kamu\n\nKamu juga bisa mulai trading sekarang di:  \n✅ Cryptocurrency  \n✅ Komoditas  \n✅ Saham  \n✅ Indeks  \nDan masih banyak lagi!  \n\n📌 Keunggulan Trading Manual:  \n- Kamu bisa memilih pasangan mata uang tertentu dan timeframe yang dibutuhkan, kapan saja dan di hari apa aja, ini memungkinkan kamu untuk trading dan meningkatkan saldo dengan fleksibel.  \n- Lebih cepat dan efisien, karena kamu bisa segera buat keputusan dari berita global dan pilih aset dengan volatilitas tertinggi.  \n- Dengan trading manual, kamu bisa dapat akses ke analisis yang kuat untuk buat keputusan yang lebih tepat dan meminimal risiko!  \n\n📢 Cara askes Trading Manual:  \nUntuk dapat akses ke trading manual, kamu perlu deposit kedua sebesar $30 ke akun trading kamu (deposit ini harus benar-benar yang kedua, jika saldo kamu sudah $30, itu tidak dihitung).  \n\nSetelah melakukan deposit, kirimkan ID pengguna khusus ({user_id}) dan tangkapan layar (screenshot) kepada manajer di (@bot_helpmanager). Setelah itu, kamu akan dapat akses ke fitur trading manual. 🚀",
}


manual_trading_button = {
    "RU": "Получить доступ к ручному трейдингу",
    "EN": "Get access to manual trading",
    "ES": "Obtener acceso al trading manual",
    "AR": "احصل على الوصول إلى التداول اليدوي",
    "TR": "Manuel ticarete erişim al",
    "ID": "Dapatkan akses ke perdagangan manual",
}


v_tools = [
    "AED/CNY OTC",
    "AUD/CAD OTC",
    "AUD/NZD OTC",
    "AUD/USD OTC",
    "CAD/CHF OTC",
    "CAD/JPY OTC",
    "EUR/CHF OTC",
    "EUR/GBP OTC",
    "EUR/TRY OTC",
    "GBP/AUD OTС",
    "GBP/JPY OTC",
    "JOD/CNY OTC",
    "LBP/USD OTC",
    "MAD/USD OTC",
    "SAR/CNY OTC",
    "USD/BRL OTC",
    "USD/CNH OTC",
    "USD/COP OTC",
    "USD/DZD OTC",
    "USD/INR OTC",
    "USD/JPY OTC",
    "USD/MXN OTC",
    "USD/MYR OTC",
    "USD/PKR OT",
    "USD/RUB OTC",
    "USD/SGD OTC",
    "USD/THB OTC",
    "CHF/NOK OTC",
    "EUR/HUF OTC",
    "EUR/JPY OTC",
    "TND/USD OTC",
    "YER/USD OTС",
    "USD/VND OTC",
    "GBP/USD OTC",
    "USD/CAD OTC",
    "QAR/CNY OTC",
    "EUR/NZD OTC",
    "USD/IDR OTC",
    "BHD/CNY OTC",
    "EUR/RUB OTC",
    "USD/BDT OTC",
    "USD/EGP OTC",
    "NZD/USD OTC",
    "EUR/USD OTC",
    "USD/CLP OTC",
    "AUD/JPY OTC",
    "USD/ARS OTC",
    "USD/PHP OTC",
    "OMR/CNY OTС",
    "CHF/JPY OTC",
    "NZD/JPY OTC",
    "AUD/CHF OTC",
    "USD/CHF OTC",
]
c_tools = [
    "Cardano OTС",
    "Avalanche OTC",
    "Bitcoin ETF OTC",
    "Chainlink OTC",
    "Polygon OTC",
    "Toncoin OTC",
    "TRON OTC",
    "Ethereum OTC",
    "Bitcoin OTC",
    "Ripple OTC",
    "Litecoin OTC",
    "BNB OTC",
    "Solana OTC",
    "Polkadot OTC",
    "Dogecoin OTC",
]
s_tools = [
    "Brent Oil OTC",
    "WTI Crude Oil OTC",
    "Silver OTC",
    "Gold OTC",
    "Natural Gas OTC",
    "Palladium spot OTC",
    "Platinum spot OTC",
]
a_tools = [
    "Apple OTC",
    "American Express OTC",
    "Intel OTC",
    "Pfizer Inc OTC",
    "Citigroup Inc OTC",
    "TWITTER OTC",
    "McDonald's OTC",
    "VISA OTC",
    "Microsoft OTC",
    "Alibaba OTC",
    "Johnson & Johnson OTС",
    "Cisco OTC",
    "Boeing Company OTC",
    "Amazon OTC",
    "FedEx OTC",
    "Tesla OTC",
    "ExxonMobil OTC",
    "Netflix OTC",
    "FACEBOOK INC OTC",
]
i_tools = [
    "AUS 200 OTC",
    "100GBP OTC",
    "D30EUR OTC",
    "DJI30 OTC",
    "E35EUR OTC",
    "E50EUR OTC",
    "F40EUR OTC",
    "JPN225 OTC",
    "US100 OTC",
    "SP500 OTC",
]

TRADER_TOOLS = {
    "RU": {
        "Валюты": {
            "tools": v_tools,
            "time": tiime_options["RU"],
        },
        "Криптовалюты": {
            "tools": c_tools,
            "time": tiime_options["RU"],
            "image": "",
        },
        "Сырьевые товары": {
            "tools": s_tools,
            "time": tiime_options["RU"],
            "image": "",
        },
        "Акции": {
            "tools": a_tools,
            "time": tiime_options["RU"],
        },
        "Индексы": {
            "tools": i_tools,
            "time": tiime_options["RU"],
        },
    },
    "EN": {
        "Currencies": {
            "tools": v_tools,
            "time": tiime_options["EN"],
        },
        "Cryptocurrencies": {
            "tools": c_tools,
            "time": tiime_options["EN"],
            "image": "",
        },
        "Commodities": {
            "tools": s_tools,
            "time": tiime_options["EN"],
            "image": "",
        },
        "Stocks": {
            "tools": a_tools,
            "time": tiime_options["EN"],
        },
        "Indices": {
            "tools": i_tools,
            "time": tiime_options["EN"],
        },
    },
    "ES": {
        "Monedas": {
            "tools": v_tools,
            "time": tiime_options["ES"],
        },
        "Criptomonedas": {
            "tools": c_tools,
            "time": tiime_options["ES"],
            "image": "",
        },
        "Materias primas": {
            "tools": s_tools,
            "time": tiime_options["ES"],
            "image": "",
        },
        "Acciones": {
            "tools": a_tools,
            "time": tiime_options["ES"],
        },
        "Índices": {
            "tools": i_tools,
            "time": tiime_options["ES"],
        },
    },
    "AR": {
        "العملات": {
            "tools": v_tools,
            "time": tiime_options["AR"],
        },
        "العملات المشفرة": {
            "tools": c_tools,
            "time": tiime_options["AR"],
            "image": "",
        },
        "المواد الخام": {
            "tools": s_tools,
            "time": tiime_options["AR"],
            "image": "",
        },
        "الأسهم": {
            "tools": a_tools,
            "time": tiime_options["AR"],
        },
        " مؤشرات البورصة": {
            "tools": i_tools,
            "time": tiime_options["AR"],
        },
    },
    "TR": {
        "Para Birimleri": {
            "tools": v_tools,
            "time": tiime_options["TR"],
        },
        "Kripto Paralar": {
            "tools": c_tools,
            "time": tiime_options["TR"],
            "image": "",
        },
        " Emtia Ürünleri": {
            "tools": s_tools,
            "time": tiime_options["TR"],
            "image": "",
        },
        "Hisse Senetleri": {
            "tools": a_tools,
            "time": tiime_options["TR"],
        },
        "Endeksler": {
            "tools": i_tools,
            "time": tiime_options["TR"],
        },
    },
    "ID": {
        "Mata Uang": {
            "tools": v_tools,
            "time": tiime_options["ID"],
        },
        "Mata uang kripto": {
            "tools": c_tools,
            "time": tiime_options["ID"],
            "image": "",
        },
        "Komoditas": {
            "tools": s_tools,
            "time": tiime_options["ID"],
            "image": "",
        },
        "Saham": {
            "tools": a_tools,
            "time": tiime_options["ID"],
        },
        "Indeks": {
            "tools": i_tools,
            "time": tiime_options["ID"],
        },
    },
}


